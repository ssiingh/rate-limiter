...
Set the admin API key in your environment variables

```env
ADMIN_API_KEY=your-secret-key
```

Access admin endpoints with HTTP Header:

```bash
curl -H "x-admin-key: change-me-admin-key" http://localhost:3000/admin/keys
```
...
1. **Check if request is allowed**:
   ```bash
   curl -X POST http://localhost:3000/api/ratelimit/check \
...
2. **If rate limited, wait and configure higher limits**:
   ...
   curl -X POST http://localhost:3000/api/ratelimit/config/keys/user:123 \
     -H "Content-Type: application/json" \
...
curl -X POST http://localhost:3000/api/ratelimit/config/default \
  -H "Content-Type: application/json" \
  -d '{"capacity":20,"refillRate":5}'

# Set API endpoint pattern limits
curl -X POST http://localhost:3000/api/ratelimit/config/patterns/api:v1:* \
  -H "Content-Type: application/json" \
  -d '{"capacity":100,"refillRate":20}'

# Reload to apply changes
curl -X POST http://localhost:3000/api/ratelimit/config/reload
```
...
curl -X POST http://localhost:3000/api/performance/baseline \
  -H "Content-Type: application/json" \
  -d '{
...
curl -X POST http://localhost:3000/api/performance/regression/analyze \
  -H "Content-Type: application/json" \
  -d '{
    "testName": "daily-load-test",
    "timestamp": "'$(date -u +%Y-%m-%dT%H:%M:%SZ)'",
...
health=$(curl -s -o /dev/null -w "%{http_code}" http://localhost:3000/actuator/health)
...
response=$(curl -s -w "%{http_code}" -X POST http://localhost:3000/api/ratelimit/check \
  -H "Content-Type: application/json" \
  -d '{"key":"user:123","tokens":1}')
